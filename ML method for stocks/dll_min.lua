...
function Tail()		--основные вычисления
...
--заполнение таблицы 
	SetCell(TableID, 2, 1, ServerDate)
	SetCell(TableID, 2, 2, ServerTime)
	SetCell(TableID, 2, 3, BID)
	SetCell(TableID, 2, 4, BIDDEPTH)
	SetCell(TableID, 2, 5, BIDDEPTHT)
	SetCell(TableID, 2, 6, NUMBIDS)
	SetCell(TableID, 2, 7, OFFER)
	SetCell(TableID, 2, 8, OFFERDEPTH)
	SetCell(TableID, 2, 9, OFFERDEPTHT)
	SetCell(TableID, 2, 10, NUMOFFERS)
	SetCell(TableID, 2, 11, OPEN)
	SetCell(TableID, 2, 12, HIGH)
	SetCell(TableID, 2, 13, LOW)
	SetCell(TableID, 2, 14, LAST)
	SetCell(TableID, 2, 15, CHANGE)
...
--вывод данных .txt
function WriteToEndofFile_min(sFile, sDataString_min)	--запись в логфайл указанную строку
	...
	sDataString_min = ServerDate.." "..ServerTime.."\n"
